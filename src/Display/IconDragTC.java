...
public class IconDragTC extends AnchorPane{
    ...
    private final IconDragTC self;
    boolean NameX = false;
    boolean ValX = false;
    public IconDragTC(ResourceBundle resourceBundle) {
    
    FXMLLoader fxmlLoader = new FXMLLoader(
    getClass().getResource("/Display/FXMLs/IconoMovibleTC.fxml"),resourceBundle
    );
...
{   buildNodeEventHandlers();
    ...
    SecondCondLink_Handle.setOnDragDetected(mLeftLinkHandleDragDetected);
    FirstCondLink_Handle.setOnDragDetected(mRightLinkHandleDragDetected);
    SecondCondLink_Handle.setOnDragDropped(mLeftLinkHandleDragDropped);
    FirstCondLink_Handle.setOnDragDropped(mRightLinkHandleDragDropped);
...
timer.scheduleAtFixedRate(new TimerTask() {
  @Override
  public void run() {
     String valor = FirstCondition_Handle.getText();
            switch(mType)
            {
            case Entero:
            ...
            if (!ColeccionDatos.ValorItem.equals(FirstCondition_Handle.getText())){           
            ColeccionDatos.ValorItem = FirstCondition_Handle.getText();
            }if (ValX && NameX) {
        switch(mType){
...
timer.scheduleAtFixedRate(new TimerTask() {
  @Override
  public void run() {
     String valor = SecondCond_Handle.getText();
            switch(mType)
            {
            case Entero:
                if(!valor.matches("[0-9]*[^.]")|| valor.isEmpty())
            ...
            if (!ColeccionDatos.ValorItem.equals(SecondCond_Handle.getText())){           
            ColeccionDatos.ValorItem = SecondCond_Handle.getText();
            }if (ValX && NameX) {
        switch(mType){
...
public void ValueDisable(boolean disable,String input){if (input =="SecondValue_Handle") {
        SecondCond_Handle.setDisable(disable);
    }else{FirstCondition_Handle.setDisable(disable);}
  }
